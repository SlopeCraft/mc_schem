...
pub const OLD_BLOCK_ID: [&'static str; 256] = [
    "air",
    "stone",
    "grass",
    "dirt",
...
const VALID_DAMAGE_LUT: [u16; 256] = [
    0b00000000000001,
    0b00000001111111,
    0b00000000000001,
    0b00000000000111,
...
pub fn is_damage_valid(id: u8, damage: u8) -> Result<(), OldBlockParseError> {
    ...
    if damage >= 16 {
        //minecraft uses only first 4 bits in damage value
    ...
    Err(OldBlockParseError::DamageNotDefinedForThisBlock { id, damage })
}
...
impl Block {
    ...
    pub fn from_old(id: u8, damage: u8, version: DataVersion) -> Result<Block, OldBlockParseError> {
        ...
        if [35, 159, 95, 171, 172, 251, 252].contains(&id) {
            //wool, hardened clay, carpet, stained-glass, concrete, concrete powder
        ...
        if [50, 75, 76].contains(&id) {
            // torch, redstone torch and unlit redstone torch
        ...
        if id == 31 {
            //grass (minecraft:tallgrass in 1.12
        ...
        if [29, 33].contains(&id) {
            //piston and sticky piston (main body)
        ...
        if [34, 36].contains(&id) {
            //piston_head and piston_extension (b36)
            let facing = index_to_piston_facing(damage & 0b111);
            debug_assert!(!facing.is_empty());
        ...
        if [151, 178].contains(&id) {
            // day light detector
            block.set_property("power", &damage);
            return Ok(block);
        }

        if [59, 141, 142, 207].contains(&id) {
            //crops
            block.set_property("age", &damage);
            return Ok(block);
        }
        ...
        if [176, 63].contains(&id) {
            //minecraft:standing_banner and standing_sign
            block.set_property("rotation", &damage);
            return Ok(block);
        }
        if [177, 68].contains(&id) {
            //minecraft:wall_banner and wall_sign
        ...
        if [64, 71, 193, 194, 195, 196, 197].contains(&id) {
            // doors
        ...
        if [27, 28, 157].contains(&id) {
            // golden, detector and activation rails
        ...
        if [54, 146, 65, 61, 62, 130].contains(&id) {
            //chest, trapped chest, ladder, furnaces
            debug_assert!(damage >= 2 && damage <= 5);
            let facing = index_to_piston_facing(damage);
            debug_assert!(!facing.is_empty());
            block.set_property("facing", facing);
        ...
        if [70, 72].contains(&id) {
            //wooden and stone pressure plate
        ...
        if [147, 148].contains(&id) {
            //weighted pressure plate
            block.set_property("power", &damage);
            return Ok(block);
        }

        if [77, 143].contains(&id) {
            //buttons
        ...
        if [86, 91].contains(&id) {
            //pumpkin and pumpkin light
        ...
        if [107, 183, 184, 185, 186, 187].contains(&id) {
            //fence gate
        ...
        if id == 117 {
            ...
            for bottle in 0..3 {
                ...
                block
                    .attributes
                    .insert(format!("has_bottle_{}", bottle), has_bottle.to_string());
            }
            return Ok(block);
        }
        if id == 118 {
        ...
        if [170, 202, 216].contains(&id) {
            //hay bale, purpur pillar, bone block
        ...
        Err(OldBlockParseError::NotImplemented {
            id,
            damage,
            version,
        })
    ...
    pub fn fix_block_property_with_block_entity(
        &self,
        _id: u8,
        _damage: u8,
        _be: &BlockEntity,
    ) -> Result<Option<Block>, OldBlockParseError> {
